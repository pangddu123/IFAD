...
def calculate_anomaly_scores(differ_datas):
    """计算每个点的异常得分（重建误差的平方和）"""
    # 假设differ_datas是一个DataFrame，每行代表一个时间点，每列代表一个变量
    anomaly_scores = np.sum(differ_datas.values ** 2, axis=1)
    return anomaly_scores



def time_test(params, strategy_params,i,anomaly_scores):
    ...
    test_set = Data.create_dataset(opt['datasets']['test'], 'test',anomaly_scores=anomaly_scores)
    test_loader = Data.create_dataloader(test_set, opt['datasets']['test'], 'test')
    params['row_num'] = test_set.row_num
    params['col_num'] = test_set.col_num
    ...
    if all_datas.shape[0] > params['row_num']:
        all_datas = all_datas[:params['row_num']]
    if sr_datas.shape[0] > params['row_num']:
